...
final class WOOBE_BULKOPERATIONS extends WOOBE_EXT {
    ...
    public function woobe_ext_scripts() {
        ...
        wp_enqueue_script('woobe_ext_' . $this->slug . '-2', $this->get_ext_link() . 'assets/js/' . $this->slug . '-2.js');
        wp_enqueue_script('woobe_ext_' . $this->slug . '-3', $this->get_ext_link() . 'assets/js/' . $this->slug . '-3.js');
        wp_enqueue_script('woobe_ext_' . $this->slug . '-4', $this->get_ext_link() . 'assets/js/' . $this->slug . '-4.js');
        wp_enqueue_script('woobe_ext_' . $this->slug . '-5', $this->get_ext_link() . 'assets/js/' . $this->slug . '-5.js');
        wp_enqueue_script('woobe_ext_' . $this->slug . '-6', $this->get_ext_link() . 'assets/js/' . $this->slug . '-6.js');
        wp_enqueue_script('woobe_ext_' . $this->slug . '-7', $this->get_ext_link() . 'assets/js/' . $this->slug . '-7.js');
        wp_enqueue_style('woobe_ext_' . $this->slug, $this->get_ext_link() . 'assets/css/' . $this->slug . '.css');
        ?>
        <script>
            lang.<?php echo $this->slug ?> = {};
            lang.<?php echo $this->slug ?>.going = '<?php _e('ATTENTION: Variations Advanced Bulk Operation is going', 'woo-bulk-editor') ?>';
            lang.<?php echo $this->slug ?>.finished = '<?php _e('Variations Advanced Bulk Operation is finished', 'woo-bulk-editor') ?>';
            lang.<?php echo $this->slug ?>.finished2 = '<?php _e('Attaching of the default combination for the products variations is finished!', 'woo-bulk-editor') ?>';
            lang.<?php echo $this->slug ?>.finished3 = '<?php _e('Deleting of the products variations is finished', 'woo-bulk-editor') ?>';
            lang.<?php echo $this->slug ?>.finished4 = '<?php _e('Ordering of the products variations is finished', 'woo-bulk-editor') ?>';
            lang.<?php echo $this->slug ?>.finished5 = '<?php _e('Swap of variations is finished', 'woo-bulk-editor') ?>';
            lang.<?php echo $this->slug ?>.finished6 = '<?php _e('Attaching of the products variations is finished', 'woo-bulk-editor') ?>';
            lang.<?php echo $this->slug ?>.generating = '<?php _e('Generating possible combinations', 'woo-bulk-editor') ?>';
            lang.<?php echo $this->slug ?>.generated = '<?php _e('Possible combinations been generated.', 'woo-bulk-editor') ?>';
            lang.<?php echo $this->slug ?>.no_combinations = '<?php _e('Combination(s) not selected!', 'woo-bulk-editor') ?>';
            lang.<?php echo $this->slug ?>.not_selected_var = '<?php _e('variation is not selected', 'woo-bulk-editor') ?>';
            lang.<?php echo $this->slug ?>.no_vars = '<?php _e('the product has no variations', 'woo-bulk-editor') ?>';
        </script>
        <?php
    }
    ...
    public function woobe_tools_panel_buttons() {
        ?>
        <a href="#" class="button button-secondary woobe_tools_panel_newvars_btn" title="<?php _e('Variations Advanced Bulk Operations', 'woo-bulk-editor') ?>"></a>
        <?php
    }
    ...
    public function woobe_bulkoperations_get_att_terms() {
        die(json_encode(WOOBE_HELPER::get_taxonomies_terms_hierarchy($_REQUEST['attribute'])));
    }
    ...
    public function woobe_bulkoperations_apply_combinations() {
        ...
        $combinations = $_REQUEST['combinations'];
        ...
        if (!empty($products_ids) AND ! empty($combinations)) {
            ...
            foreach ($combinations as $k => $comb) {
                if (!empty($comb)) {
                    foreach ($comb as $term_id) {
                        $possible_attributes[$k][$terms[$term_id]['taxonomy']] = $terms[$term_id]['slug'];
                    }
                }
            }
            ...
            if (!empty($possible_attributes)) {
                ...
                foreach ($products_ids as $product_id) {
                    ...
                    foreach ($set_product_attributes as $field_key => $value) {
                        $this->products->set_product_attributes($product_id, $field_key, $value, 'append');
                    }
                ...
                if (!empty($childrens)) {
                    ...
                    if (!empty($available_variations)) {
                        foreach ($available_variations as $var) {
                            ...
                            //***
                            if ($num > -1) {
                                $this->products->update_page_field($var['variation_id'], 'menu_order', $num);
                            }
                        }
                    }
                }
    ...
    public function woobe_bulkoperations_swap() {
        ...
        if ($do) {
            $from_att = $_REQUEST['from']['attribute'];
            $to_att = $_REQUEST['to']['attribute'];
                        ...
                        foreach ($childrens as $child_id) {
                            ...
                            if (isset($available_variations['attributes']) AND ! empty($available_variations['attributes'])) {
                                if (isset($available_variations['attributes']['attribute_' . $from_att])) {
                                    if ($available_variations['attributes']['attribute_' . $from_att] === $_REQUEST['from']['term']) {
                                        ...
                                        $possible_attributes['attribute_' . $to_att] = $_REQUEST['to']['term'];
                                        ...
                                        if (!in_array($_REQUEST['to']['term'], $parent_terms)) {
                                            $p_terms = array_keys($parent_terms);
                                            $t = get_term_by('slug', $_REQUEST['to']['term'], $to_att);
                                            $p_terms[] = $t->term_id;
    ...
    public function woobe_bulkoperations_attaching() {
        ...
        $selected_attribute = $_REQUEST['selected_attribute'];
        $attaching_att = $_REQUEST['attaching_att'];
        $products_ids = $_REQUEST['products_ids'];
                                    ...
                                    foreach ($attaching_att as $set) {
                                        ...
                                        if ($att_set_now_keys === $set_keys) {
                                            ...
                                            if ($att_set_now_vals === $set_vals AND $set['value'] !== 'woobe-ignore') {
                                                $possible_attributes['attribute_' . $selected_attribute] = $set['value'];

                                                //if such attribute not selected in the parent product Attributes tab lets attach it here
                                                $parent_terms = wc_get_product_terms($product_id, $selected_attribute, array('fields' => 'slugs'));
                                                if (!in_array($set['value'], $parent_terms)) {
                                                    $p_terms = array_keys($parent_terms);
                                                    $t = get_term_by('slug', $set['value'], $selected_attribute);
                                                    $p_terms[] = $t->term_id;

                                                    $this->products->update_page_field($product_id, $selected_attribute, $p_terms);
                                                }
                        ...
                        if(is_array($meta)){
                            foreach ($meta as $pa_key=>$item) {
                                if($item['name']==$vis['attribute']){
                                    $meta[$pa_key]['is_visible'] = $vis['is_visible'];
                                    $meta[$pa_key]['is_variation'] = $vis['is_variation'];
                                }
                            }                            
                        }
                    }
