...
<div class="col-lg-12">
    <div style="display: none;" id="woobe_show_variations_mode_export">
        ...
        <?php if (!empty($combination_attributes)): ?>
            ...
            <select id="woobe_bulk_combination_attributes_export" multiple="" class="chosen-select" style="width: 350px;" data-placeholder="<?php _e('select combination of attributes', 'woo-bulk-editor') ?>">
                <?php foreach ($combination_attributes as $a) : ?>
                    <option value="pa_<?php echo $a->attribute_name ?>"><?php echo $a->attribute_label ?></option>
                <?php endforeach; ?>
            </select>
            <select id="woobe_bulk_combination_attributes_export_behavior"  class="chosen-select" style="width: 100px;" data-placeholder="<?php _e('select behavior', 'woo-bulk-editor') ?>">
                <option value="1" ><?php _e('In', 'woo-bulk-editor') ?></option>
                <option value="0" ><?php _e('Not in', 'woo-bulk-editor') ?></option>
            </select>                      
            &nbsp;<a href="javascript: woobe_bulk_add_combination_to_apply_export();void(0);" id="woobe_bulk_add_combination_to_apply_export" class="button button-primary button"><?php _e('Add attributes combination', 'woo-bulk-editor') ?></a>

            <br />
            <form>
                <ul id="woobe_bulk_to_var_combinations_apply_export"></ul>
            </form>
            <small style="font-style: italic;"><?php _e('Select combination(s) of attributes you want (in) or do not want (not in) export if you need it. If leave empty will be exported all combinations. Combinations should be strongly exact!', 'woo-bulk-editor') ?></small>
            <br />
            <br />
            <hr />
            <br />
