...
class WOOBE_FILTER_PROFILES extends WOOBE_PROFILES {
        ...
        if (!$this->get()) {
            ...
            $this->create(array(
                'product_type' => 'variable'
                    ), __('Variable products', 'woo-bulk-editor'), 'default');
        }
    }
    ...
    public function load_profile() {
        ...
        if (!empty($profile)) {
            if (isset($profile['data']) AND ! empty($profile['data'])) {
                $this->storage->set_val('woobe_filter_' . $_REQUEST['profile_key'], $profile['data']);
            } else {
                die('-1');
            }
        } else {
    ...
    public function create_profile() {
        ...
        if (!empty($profile_title)) {
            ...
            $filter_current_key = $_REQUEST['filter_current_key'];
