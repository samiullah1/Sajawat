...
class WOOBE_PROFILES {
    ...
    protected $non_deletable_profiles = array('default', 'for_variations', 'for_stock', 'for_prices', 'for_attachments');
    ...
    protected function init_constructor_data() {
        ...
        if (!$this->get()) {
            ...
            $this->create(array(
                ...
                'gallery'
                    ), __('Default', 'woo-bulk-editor'), 'default');
            ...
            $this->create(array(
                ...
                'download_limit', 'download_expiry', 'tax_class', 'backorders', 'weight', 'height', 'width', 'length', 'post_parent'
                    ), __('For variations fields only', 'woo-bulk-editor'), 'for_variations');

            $this->create(array(
                '__checker', 'ID', '_thumbnail_id', 'post_title', 'manage_stock', 'stock_quantity', 'stock_status'), __('Stock', 'woo-bulk-editor'), 'for_stock');


            $this->create(array(
                '__checker', 'ID', '_thumbnail_id', 'post_title', 'regular_price', 'sale_price', 'date_on_sale_from', 'date_on_sale_to'), __('Prices', 'woo-bulk-editor'), 'for_prices');


            $this->create(array(
                '__checker', 'ID', '_thumbnail_id', 'post_title', 'downloadable', 'download_files', 'download_limit', 'download_expiry', 'cross_sell_ids', 'upsell_ids', 'grouped_ids'), __('Downloads, Cross-sells, Up-sells, Grouped', 'woo-bulk-editor'), 'for_attachments');
        }
    }
    ...
    public function create_profile() {
        ...
        $profile_title = trim(htmlentities($_REQUEST['profile_title'], ENT_NOQUOTES));
    ...
    public function delete_profile() {
        ...
        $this->delete($_REQUEST['profile_key']);
